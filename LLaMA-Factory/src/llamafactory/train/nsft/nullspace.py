...
from torch_npu.optim import Adamw
...
class AdamWns(AdamW):
    def __init__(self, 
                 ...
                 lr: float | torch.Tensor = 0.001, 
                 betas: torch.Tuple[float] = ..., 
                 eps: float = 1e-8, 
        ...
        super().__init__(
            ...
            fused=fused)
        self.nullspace:List[Tensor] = torch.load(nullspace_path, map_location=torch.device("cpu"))     
    ...
    def _init_group(
        ...
        for i, p in enumerate(group["params"]):
            ...
            if len(state) == 0:
                ...
                # state["null_space"] = torch.zeros_like(
                #     p, memory_format=torch.preserve_format
                # ).copy_(self.nullspace[i].to(p.device))
                state["null_spaces"] = self.nullspace[i].contiguous(memory_format=torch.preserve_format
            ...
            null_spaces.append(state["null_spaces"])
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        if capturable or differentiable:
            ...
            param.add_(-step_size * torch.matmul(null_space, exp_avg.div(denom)))
...
def _multi_tensor_adamw(
    ...
    *,
    ...
    grouped_tensors = Optimizer._group_tensors_by_device_and_dtype([
        params, grads, exp_avgs, exp_avg_sqs, max_exp_avg_sqs, state_steps, null_spaces])
    for ((
        device_params,
        device_grads,
        device_exp_avgs,
        ...
        if capturable:
            ...
            update_device_params = [torch.matmul(n, x / y) for (x, y, n) in zip(device_exp_avgs, exp_avg_sq_sqrt, device_null_spaces)]
            torch._foreach_add_(device_params, update_device_params)
        else:
            ...
            update_device_params = [z * torch.matmul(n, x / y) for (x, y, z, n) in zip(device_exp_avgs, exp_avg_sq_sqrt, step_size, device_null_spaces)]
            torch._foreach_add_(device_params, update_device_params)
