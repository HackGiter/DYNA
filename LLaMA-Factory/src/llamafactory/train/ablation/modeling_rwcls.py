...
class rWCLS(rWCLSPreTrainedModel):
    def __init__(
        ...
        # self.conv_kernel[-1] = self.conv_kernel[-1] / 2
        # self.conv_kernel[-2] = self.conv_kernel[-2] / 2
        self.conv_kernel[0] = self.conv_kernel[0] / 2
        self.conv_kernel[1] = self.conv_kernel[1] / 2
        ...
        self.epoch = config.epoch * self.gradient_accumulation
    ...
    def forward(
        ...
        #     masked_loss = loss.detach().masked_fill(shift_labels.view(-1).eq(self.config.eos), 0.0).unsqueeze(0)
        #     local_conv_loss:torch.Tensor = F.conv1d(masked_loss, self.conv_kernel, padding='same', groups=1).squeeze(0)
        ...
        #     # masks = torch.zeros_like(loss, dtype=torch.int32, requires_grad=False)

        #     if self.cnt > 10:
        #         token_masks.scatter_(-1, local_top_indices, local_top_values * self.alpha)
        ...
        #         # masks.scatter_(-1, local_top_indices, 1)
        #         # masks.masked_fill_(local_conv_loss < self.global_top_tokens[-1], 0)
        #         # masks.masked_fill_(loss.detach() < 1.0, 0)
        ...
        # loss = loss.masked_fill(masks == 1, 0.0)
        # loss = loss.sum() / (shift_labels.numel() - shift_labels.eq(self.ignore_index).sum() - (masks == 1).sum())

        # with torch.no_grad():
        #     numel = shift_labels.not_equal(self.ignore_index).sum().item()
        #     # self.global_numel = (numel + self.global_numel * self.cnt) / (self.cnt + 1)
        #     self.cnt += 1

        #     masked_numel = int(numel * self.local_numel_ratio)
        #     # masks = torch.zeros_like(loss, dtype=torch.int32, requires_grad=False)
        #     masks = torch.ones_like(loss, requires_grad=False)

        #     # if self.cnt > 10:
        #     non_zero_indices = torch.nonzero(shift_labels.not_equal(self.ignore_index), as_tuple=True)[0]
        #     non_zero_indices = non_zero_indices[torch.randperm(numel)[:masked_numel]]
        #         # _, non_zero_indices = torch.topk(loss.detach(),
        #         #                                             k=masked_numel,
        #         #                                             dim=-1,
        #         #                                             largest=False)
        #     non_zero_values = torch.rand_like(non_zero_indices, dtype=masks.dtype, requires_grad=False)
        #     # masks.scatter_(-1, non_zero_indices, 1)
        #     masks.scatter_(-1, non_zero_indices, non_zero_values)
        ...
        # # loss = loss.masked_fill(masks == 1, 0.0)
        # loss = loss * masks
        # # loss = loss.sum() / (shift_labels.numel() - shift_labels.eq(self.ignore_index).sum() - masked_numel)
        # loss = loss.sum() / (shift_labels.numel() - shift_labels.eq(self.ignore_index).sum())

        with torch.no_grad():
            numel = shift_labels.not_equal(self.ignore_index).sum().item()
            # k = max(1, int(numel * 0.01))
            k = max(1, int(numel * 0.05))

            _, topk_indices = torch.topk(loss.detach(), k=k, dim=-1)

            masked = torch.zeros_like(loss.detach(), dtype=torch.bool)
            #masked[topk_indices[k // 2:]] = True
            #masked[topk_indices] = True

        loss = loss.masked_fill(masked, 0.0)
        # loss = loss.sum() / (shift_labels.numel() - shift_labels.eq(self.ignore_index).sum() - (k // 2))
        loss = loss.sum() / (shift_labels.numel() - shift_labels.eq(self.ignore_index).sum() - k)
