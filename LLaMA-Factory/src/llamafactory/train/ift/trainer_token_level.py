...
class CustomSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def __init__(
        ...
        self.p_extra = kwargs.pop("p_extra", 0.12)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if labels is not None:
            ...
            loss:torch.Tensor = (self.p_extra) * loss_fct(shift_logits, shift_probs) + (1 - self.p_extra) * loss_fct(shift_logits, shift_labels)
            loss.masked_fill_(shift_masks, 0.0)
            loss = loss.sum() / (shift_masks.numel() - shift_masks.long().sum() + 1e-8)
        else:
